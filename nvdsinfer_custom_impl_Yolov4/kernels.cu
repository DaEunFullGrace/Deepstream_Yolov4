...
__global__ void gpuYoloLayerV4(const float* input, float* output, 
                            const uint gridH, const uint gridW, const uint numOutputClasses,
                            const uint numBBoxes, const uint new_coords, const float scale) {
    
    uint x_id = blockIdx.x * blockDim.x + threadIdx.x;
    uint y_id = blockIdx.y * blockDim.y + threadIdx.y;
    uint z_id = blockIdx.z * blockDim.z + threadIdx.z;

    if ((x_id >= gridW) || (y_id >= gridH) || (z_id >= numBBoxes))
    {
        return;
    }

    const int numGridCells = gridH * gridW;
    const int bbindex = y_id * gridW + x_id;

    float alpha = scale;
    float beta = -0.5 * (scale - 1);
...
__global__ void gpuYoloLayer(const float* input, float* output, 
...
cudaError_t cudaYoloLayer(const void* input, void* output, const uint& batchSize, 
                            const uint& gridH, const uint& gridW,
                            const uint& numOutputClasses, const uint& numBBoxes,
                            uint64_t outputSize, cudaStream_t stream, const uint coords, 
                            const float scale, const uint type);
                            ...
                            const float scale, const uint type)
{
    ...
    dim3 number_of_blocks((gridW / threads_per_block.x) + 1,
                          (gridH / threads_per_block.y) + 1,
                          (numBBoxes / threads_per_block.z) + 1);

    if (type == 1) {
        for (unsigned int batch = 0; batch < batchSize; ++batch) {
            gpuYoloLayerV4<<<number_of_blocks, threads_per_block, 0, stream>>>(
                reinterpret_cast<const float*>(input) + (batch * outputSize),
                reinterpret_cast<float*>(output) + (batch * outputSize), gridH, gridW, 
                numOutputClasses, numBBoxes, coords, scale);
        }
    } else {
        for (unsigned int batch = 0; batch < batchSize; ++batch)
        {
